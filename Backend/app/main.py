...
def get_post(id: int, db: Session = Depends(get_db)):
    ...
    pos_at=post.post_at.isoformat()
    new_pos=PostSch.numi(title=post.title,body=post.body,user_name=use.name,id=post.id,u_id=use.u_id,photo=photo_byte,modify_at=pos_at)
...
def retur_user(token,db: Session = Depends(get_db)):
    ...
    if not alu:
         raise HTTPException(status_code=404, detail="User Not Found or Incorrect Password")
    alu1=userSch.user_id_response(email=alu.email,name=alu.name,role=alu.role,u_id=alu.u_id)
    return alu1
...
def up_comment(id: str,input:commmsh.CommentInput,db: Session = Depends(get_db)):
    name="comment"+id
    u_id=int(input.u_id)
    user1=db.query(user.user).filter(user.user.u_id==u_id).first()
    if not user1:
        return "null"
